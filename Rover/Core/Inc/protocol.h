...
// Подструктура 2: Настройка ПИД (Master + Slave)
typedef struct {
    uint8_t motor_id;       // 1 байт (0 = левый борт, 1 = правый борт)
    float kp;               // 4 байта
    float ki;               // 4 байта
    float kd;               // 4 байта
    float kff;              // 4 байта — Feed-Forward коэффициент
    uint16_t min_pwm;       // 2 байта — порог страгивания (мёртвая зона ШИМ)
    uint8_t kp_sync_x100;   // 1 байт  — kp_sync × 100 (0–255 → 0.00–2.55)
} MsgPidTune;
